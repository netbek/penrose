...
Penrose.prototype = {
  ...
  createDerivative: function(style, src, dist) {
    ...
    let distResolved = this.resolvePath(dist);
    ...
    // If dist format is given, then set correct file extension.
      ...
      distResolved = replaceExt(
        distResolved,
        '.' + FORMAT_EXTNAME_MAP[distFormat]
      );
    }

    return mkdirpAsync(path.dirname(distResolved)).then(() => {
      let transformer = sharp(srcResolved);

      style.actions.forEach(action => {
        if (action.name === RESIZE) {
          transformer = transformer.resize(action.width, action.height);
        } else {
      ...
      return transformer.toFile(distResolved).then(() => distResolved);
    });
  },
  /**
   *
